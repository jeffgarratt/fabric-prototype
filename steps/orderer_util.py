...
class DeliverStreamHelper(StreamHelper):
    ...
    def seekToRange(self, chainID = TEST_CHAIN_ID, start = 'Oldest', end = 'Newest'):
        ...
        envelope = bootstrap_util.create_envelope_for_msg(directory=self.directory, chainId=chainID, msg=seekInfo,
                                                          typeAsString="DELIVER_SEEK_INFO",
                                                          nodeAdminTuple=self.nodeAdminTuple)
        self.send(envelope)
