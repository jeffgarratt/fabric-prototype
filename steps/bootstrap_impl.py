...
def step_impl(context, ordererConfigAdmin, consortiumsConfigUpdateName, configName, ordererSystemChainIdName):
    ...
    config_update = bootstrap_util.create_orderer_consortium_config_update(orderer_system_chain_id, channel_group,
                                                                           config_groups)
...
def step_impl(context, userName, peerOrgSetName):
    ...
    user.setTagValue(peerOrgSetName,
                     [directory.getOrganization(row['Organization']).name for row in context.table.rows])
...
def step_impl(context, userName, createChannelSignedConfigEnvelopeName):
    ...
    for row in context.table.rows:
        ...
        bootstrap_helper.add_signature_to_config_update_envelope(config_update_envelope,
                                                                 (user, namedAdminTuple.organization, cert))
...
def step_impl(context, userName, certAlias, configTxName, orderer):
    ...
    bootstrap_util.broadcast_channel_config_tx(context=context, certAlias=certAlias, composeService=orderer, user=user,
                                               configTxEnvelope=configTxEnvelope)
...
def step_impl(context, userName, transactionAlias, orderer):
    ...
    bootstrap_util.broadcast_channel_config_tx(context=context, certAlias=None, composeService=orderer, user=user,
...
def step_impl(context, userName, certAlias, genisisBlockName, joinChannelResult):
    ...
    proposal = endorser_util.createInvokeProposalForBDD(context, ccSpec=ccSpec, chainID="", signersCert=signersCert,
                                                        Mspid=user.tags[certAlias].organization, type="CONFIG")
    ...
    proposalResponseFutures = [endorserStub.ProcessProposal.future(signedProposal, int(timeout)) for endorserStub in
                               endorser_util.getEndorserStubs(context, composeServices=endorsers, directory=directory,
                                                              nodeAdminTuple=nodeAdminTuple)]
...
def step_impl(context, userName, proposalResponseName, proposalResponseResultCode):
    ...
    unexpectedResponses = [(composeService, proposalResponse) for composeService, proposalResponse in
                           peerToProposalResponseDict.items() if
                           str(proposalResponse.response.status) != proposalResponseResultCode]
...
def check_state(container_data, state_status, state_running):
    ...
    assert current_state_status == state_status, "Expected State:Status of {0} for service {1}, instead found {2}".format(
        state_status, container_data.composeService, current_state_status)
    assert container_data.inspect_data['State']['Running'] == ast.literal_eval(
        state_running), "Expected State:Running to be {0} for service {1}, instead found {2}".format(state_running,
                                                                                                     container_data.composeService,
                                                                                                     container_data.inspect_data[
                                                                                                         'State'][
                                                                                                         'Running'])
...
def step_impl(context, certAlias):
    ...
    for row in context.table.rows:
        nodeAdminNamedTuple = directory.registerOrdererAdminTuple(ordererBootstrapAdmin.name, "ordererBootstrapAdmin",
                                                                  row['Organization'])
...
@given(u'the ordererBootstrapAdmin takes a snapshot "{orderer_snapshot_alias}" of the orderer system')
